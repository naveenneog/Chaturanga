"""Pre-generate Indian-English DragonHD narration for the game's teachings (Azure AI Speech, AAD).

Every line the game reads aloud (piece teachings, pawn life-lessons, capture/promotion/check/
checkmate lines) is synthesized once with an expressive **DragonHD** Indian-English voice and
saved as a small mp3, plus a manifest mapping the exact text -> file. At runtime the game plays
the matching clip and falls back to the browser voice only if a clip is missing.

Voices (DragonHD honour prosody but ignore express-as styles, so expressiveness = gentle
prosody within the natural-sounding band):
  * en-IN-Arjun:DragonHDLatestNeural  — the sage narrator (most lines)
  * en-IN-Neerja:DragonHDLatestNeural — a gentler voice (pawn life-lessons, promotion)

Output: web/assets/<world>/voice/<hash>.mp3  +  web/assets/<world>/voice/voice.json

Usage: python tooling/gen_voice.py [world ...]   (default: all worlds under web/worlds)
"""
import hashlib
import html
import json
...
NARRATOR = "en-IN-Arjun:DragonHDLatestNeural"   # sage
GENTLE = "en-IN-Neerja:DragonHDLatestNeural"    # gentle
...
def synth(text, out, voice, rate="0%", pitch="0%", tries=4):
    ...
    for i in range(1, tries + 1):
        ...
        req.add_header("User-Agent", "chaturanga")
        try:
...
def main():
    ...
    for wid in worlds:
        ...
        print(f"[{wid}] manifest -> {out_dir/'voice.json'} ({len(manifest)} entries)")
    print("DONE")
